...
const GoodsCard = ({ img, title, composition, price, id, type, basket = false, goods = null, onDeleteItemFromBasket = null, onGetSum }) => {
    ...
    <>
      <NavLink to={`/catalog/${id}`}>
        <div className="goods__card__wrapper">
          <div className="goods__img">
            <img src={img} alt="" />
            <img className="goods__heart" src={onShowWishGoods() ? heartActive : heartNoActive} alt="" />
          </div>
      ...
      {basket ? (
        <div className="basket__count__container">
          ...
          <div onClick={() => onDeleteItemFromBasket(id)} className="basket__delete">
            <img src={remove} alt="" />
          </div>
        </div>
      ) : null}
